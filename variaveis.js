function criarBarra(var_nome = 'Seu Nome', var_avatar = '', var_insta_nome = 'seunome', var_insta_url = 'https://instagram.com', var_seguidores = '1,2 mil', var_publi = '132') {
  let instadata = fetch(`https://api.kr3w.gg/chatbot/${var_insta_nome}`)
    .then(response => response.json())
    .then(data => {
      let info = data.data;
      var_nome = info.name;
      var_avatar = info.avatar;
      var_seguidores = info.followers;
      var_publi = info.post;

      if (var_seguidores >= 1000) {
        var_seguidores = (var_seguidores / 1000).toFixed(1) + 'K';
      } else if (var_seguidores >= 1000000) {
        var_seguidores = (var_seguidores / 1000000).toFixed(1) + 'M';
      } else if (var_seguidores >= 1000000000) {
        var_seguidores = (var_seguidores / 1000000000).toFixed(1) + 'B';
      }

      if (var_publi >= 1000) {
        var_publi = (var_publi / 1000).toFixed(1) + 'K';
      } else if (var_publi >= 1000000) {
        var_publi = (var_publi / 1000000).toFixed(1) + 'M';
      }
    })
    .catch(err => {
